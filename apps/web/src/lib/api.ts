...
/**
 * The API's base URL, with any trailing slash removed.
 *
 * Every path passed in below starts with `/`, so a base of
 * `https://api.example.com/` produces `https://api.example.com//auth/login` —
 * and a double slash is a different path to most routers. Express answers it
 * with `Cannot POST //auth/login`, a 404 that looks like a missing endpoint
 * rather than a misconfigured variable, which is a slow thing to work out from
 * the browser end.
 *
 * The API strips trailing slashes off CORS_ORIGINS for the same reason: the
 * value is usually pasted from an address bar, and address bars add the slash.
 */
declare global {
  interface Window {
    __CIQ_CONFIG__?: { apiUrl?: string };
  }
}

/**
 * Runtime configuration wins over the compiled-in value.
 *
 * `/config.js` is generated by the web container from its own environment at
 * start-up. `import.meta.env.VITE_API_URL` is substituted by Vite at *build*
 * time — so on a container it is frozen into the bundle, and setting the
 * variable on the running service changes nothing. That is a genuinely
 * confusing failure: the value is right everywhere you look, and the app still
 * calls the old address.
 *
 * On a static host there is no container, `/config.js` 404s, this is undefined,
 * and the build-time value is used — correct there, because that host rebuilds
 * on every deploy.
 */
const CONFIGURED =
  (typeof window !== 'undefined' ? window.__CIQ_CONFIG__?.apiUrl : undefined) ||
  import.meta.env.VITE_API_URL ||
  'http://localhost:4000/api/v1';

const BASE_URL = CONFIGURED.replace(/\/+$/, '');
...
/**
 * The API mounts every route under a prefix (`API_PREFIX`, default `api/v1`),
 * so a base URL without one reaches nothing — every call 404s with
 * `Cannot GET /...`. Warned rather than corrected, because the prefix is
 * configurable and guessing it would be worse than saying so.
 */
if (!/\/api\/v\d+$/.test(BASE_URL)) {
  log.warn(
    `VITE_API_URL is "${BASE_URL}", which has no /api/v1 path. ` +
      'Unless API_PREFIX was changed, every request will 404.',
  );
}
